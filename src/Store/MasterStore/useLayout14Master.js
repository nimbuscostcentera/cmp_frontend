// store/useLayout14Master.js
import { create } from "zustand";
import axios from "axios";
import {
  AddLayout14MasterAPI,
  UpdateLayout14MasterAPI,
  DeleteLayout14MasterAPI,
} from "../../Apis/MasterApis";

const useLayout14Master = create((set, get) => ({
  layout14: [], // generic array for any layout14 master
  ...
  // Fetch all layout14 items
  fetchLayout14: async (type) => {
    set({ fetchIsLoading: true, fetchError: null, fetchIsSuccess: false });
    try {
      const res = await axios.get(`${AddLayout14MasterAPI}?type=${type}`); // type passed to API
      set({ layout14: res.data, fetchIsLoading: false, fetchIsSuccess: true });
    } catch (err) {
      set({
  ...
  // Add new item
  addLayout14: async (type, newItem) => {
    set({ addIsLoading: true, addError: null, addIsSuccess: false });
    try {
      await axios.post(AddLayout14MasterAPI, { ...newItem, type });
  ...
  // Update item
  updateLayout14: async (type, id, updatedData) => {
    set({ updateIsLoading: true, updateError: null, updateIsSuccess: false });
    try {
      await axios.put(`${UpdateLayout14MasterAPI}/${id}/`, {
        ...updatedData,
        type,
      });
  ...
  // Delete item
  deleteLayout14: async (type, id) => {
    set({ deleteIsLoading: true, deleteError: null, deleteIsSuccess: false });
    try {
      await axios.delete(`${DeleteLayout14MasterAPI}/${id}/?type=${type}`);
...
export default useLayout14Master;
