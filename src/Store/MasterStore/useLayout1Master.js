...
const useLayout1Master = create((set, get) => ({
  ...
  fetchLayout1: async (type) => {
    ...
    try {
      const res = await axios.get(AddLayout1MasterAPI(type)); // type passed to API
  ...
  updateLayout1: async (type, id, updatedData) => {
    ...
    try {
      await axios.put(`${UpdateLayout1MasterAPI(type)}/${id}/`, updatedData);
  ...
  deleteLayout1: async (type, id) => {
    ...
    try {
      await axios.delete(`${DeleteLayout1MasterAPI(type)}/${id}/`);
